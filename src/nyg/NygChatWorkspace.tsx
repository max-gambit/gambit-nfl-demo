...
const STARTERS = [
  { label: 'Explore the EDGE trade market', question: 'How has the trade market for edge rushers changed from 2016 to 2025?', detail: 'Historical transactions · complete recorded packages' },
  { label: 'Test a Brian Burns trade scenario', question: 'What if we moved Brian Burns for a 2027 second?', detail: 'A proposed return · contract calculations' },
  { label: 'Check the Giants cap accounting', question: 'How much 2026 cap space do the Giants currently have?', detail: 'Public source dates · accounting basis' },
];
...
export function NygChatWorkspace({ showLibrary = false, onOpenChat }: { showLibrary?: boolean; onOpenChat: () => void }) {
  ...
  return <div className="giants-chat-shell">
    ...
    <aside className={`gc-sidebar${railOpen ? ' is-open' : ''}`}>
      ...
      <div className="gc-sidebar-foot"><span>NEW YORK GIANTS</span><p>Public evidence.<br />Staff decisions.</p></div>
    </aside>
    <main className="gc-main">
      {showLibrary ? <div className="gc-library"><span className="gc-eyebrow">SAVED BRIEFS</span><h1>Evidence, ready to revisit.</h1><p>Briefs keep the selected answer, calculations, assumptions and source links together.</p>{saved.length === 0 ? <div className="gc-empty-note">No briefs saved yet. Use “Save as brief” below an answer in Chat.</div> : saved.map(answer => <button className="gc-library-card" key={answer.id} onClick={() => { setBriefId(answer.id); setActiveBrief(answer.id); void loadBriefData(answer.id); }}><small>{date(answer.created_at)}</small><strong>{briefTitle(answer)}</strong><span>{answer.body?.kind === 'data_analysis' ? answer.body.answer : ''}</span><b>Open brief →</b></button>)}</div> : <>
        <header className="gc-conversation-header"><div><span className="gc-eyebrow">FOOTBALL OPERATIONS</span><h1>{activeSession?.label ?? 'Research desk'}</h1></div><span className="gc-save-state">{pending ? 'Checking sources…' : turns.length ? 'Conversation saved' : 'New conversation'}</span></header>
        <div className="gc-scroll" aria-label="Conversation">
          {!sessionsLoaded || !briefsLoaded ? <p className="gc-empty-note" role="status">Opening the research desk…</p> : turns.length === 0 && !pending ? <section className="gc-welcome"><span className="gc-eyebrow">START WITH THE QUESTION</span><h2>Explore the facts.<br />Work through the scenario.</h2><p>Ask about player records, historical transactions, contract calculations or the rule behind a move.</p><div className="gc-starters">{STARTERS.map((starter, i) => <button key={starter.label} onClick={() => { setDraft(starter.question); inputRef.current?.focus(); }}><span>0{i + 1}</span><div><strong>{starter.label}</strong><small>{starter.detail}</small></div><b>↗</b></button>)}</div><p className="gc-welcome-note">Change the question or add a constraint at any point.<br />Sources and limitations travel with each answer.</p></section> : <div className="gc-turns">
            {turns.map(answer => <article key={answer.id} className="gc-turn" data-brief-id={answer.id}>
              ...
              <div className="gc-assistant"><div className="gc-answer-mark"><span>G</span><small>GAMBIT · RECORDED EVIDENCE</small></div>
