...
export function NygApp() {
  ...
  function resetPresentation() {
    setView('decision'); setTarget(DEFAULT_TARGET); setProtectedPlayers([]); setProtectedGroups(DEFAULT_GROUPS); setAssumption('');
  ...
  useEffect(() => {
    if (!focusedAction || view !== 'decision' || !window.matchMedia('(max-width: 1100px)').matches) return;
  ...
  return <div className="nyg-app" data-presenter={presenter ? 'true' : 'false'} data-qa-simulation={qaBlocked ? 'blocked' : undefined}>
    ...
    <main className="nyg-main">
      ...
      {view === 'roster' && <RosterCap roster={roster} onFocus={(action) => { setSelectedBranch('maximize_relief'); setFocusedAction(action); setView('decision'); }} decision={decision} />}
