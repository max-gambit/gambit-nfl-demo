...
import { NygChatWorkspace } from './NygChatWorkspace';
...
const NAV: Array<{ id: View; label: string }> = [
  { id: 'analysis', label: 'Chat' }, { id: 'briefing', label: 'Briefs' },
  { id: 'roster', label: 'Roster & Cap' },
  { id: 'rulebook', label: 'Rulebook' }, { id: 'settings', label: 'Sources' },
];
...
export function NygApp() {
  ...
  return <div className="nyg-app">
    <header className="nyg-header">
      <button className="nyg-wordmark" onClick={() => setView('analysis')} aria-label="Open Chat"><span className="nyg-monogram">NY</span><span><strong>GIANTS</strong><small>FOOTBALL OPERATIONS</small></span></button>
...
function Workspaces({ workspaces, onCreate, onOpenAnalysis }: { workspaces: NflWorkspaceSummary[]; onCreate: (question: string) => Promise<void>; onOpenAnalysis: (workspace: NflWorkspaceSummary | null) => void }) {
  const stages = [
    ...
    { id: 'decision', label: 'Decision', note: 'Record the staff decision.' },
  ...
  return <section className="page-shell"><div className="workspace-title-row"><PageTitle eyebrow="Workspaces · NYG" title={selected?.title ?? 'Decision workspaces'} subtitle="Keep the question, supporting history, scenarios, decision, and action plan together." /><button className="primary-button" onClick={() => setDraftOpen(true)}>+ New workspace</button></div>
    ...
    <div className="workspace-overview"><div><span className="kicker">Decision owner</span><strong>Football Operations</strong></div><div><span className="kicker">Current stage</span><strong>{selected ? stageLabel(selected.stage) : 'Question'}</strong></div><div><span className="kicker">Starting point</span><strong>Live Analysis</strong></div><button className="primary-button" onClick={() => onOpenAnalysis(selected)} disabled={!selected}>Continue in Analysis</button></div><div className="stage-track">{stages.map((stage, index) => <div key={stage.id} className={index < currentStage ? 'complete' : index === currentStage ? 'current' : ''}><span>{index < currentStage ? '✓' : index + 1}</span><strong>{stage.label}</strong><small>{stage.note}</small></div>)}</div><div className="workspace-notes"><article><span className="kicker">Decision statement</span><h3>{selected?.question ?? 'Which position, period or proposed transaction should be examined?'}</h3></article><article><span className="kicker">Next action</span><h3>Inspect the recorded transactions, comparison filters and selected Giants contract.</h3></article></div></section>;
}
...
function RosterCap({ roster }: { roster: GetCurrentNflTeamResponse | null }) {
  ...
  const commitments = rows.reduce((sum, row) => sum + (row.source_status === 'captured' ? row.cap_number_2026 ?? 0 : 0), 0);
