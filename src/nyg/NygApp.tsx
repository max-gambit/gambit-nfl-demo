...
function AnalysisModeBar({ mode, onChange }: { mode: AnalysisMode; onChange: (mode: AnalysisMode) => void }) {
  return <div className="analysis-modebar"><div><strong>Analysis</strong></div><div role="tablist" aria-label="Analysis modes"><button role="tab" aria-selected={mode === 'workspace'} className={mode === 'workspace' ? 'active' : ''} onClick={() => onChange('workspace')}>Questions</button><button role="tab" aria-selected={mode === 'cap_model'} className={mode === 'cap_model' ? 'active' : ''} onClick={() => onChange('cap_model')}>Cap analysis</button></div></div>;
}
...
function DecisionRoom(props: DecisionProps) {
  ...
  return <section className="decision-layout"><div className="decision-main"><PageTitle eyebrow="Analysis · Cap & roster" title="Create room. Preserve the football plan." subtitle="Set the target and protection rules. Every displayed dollar stays tied to a player contract row." />
    {loading && !decision && <div className="model-state" role="status"><strong>Loading the reviewed cap analysis…</strong></div>}
    {decision?.status === 'insufficient_evidence' && <div className="model-state warning" role="status"><strong>The requested target is not supported by the current contract rows.</strong><span>The largest supportable result is shown below.</span></div>}
...
function Workspaces({ onOpenDecision, decision, workspaces, onCreate }: { onOpenDecision: () => void; decision: NflCapRosterDecisionResponse | null; workspaces: NflWorkspaceSummary[]; onCreate: (question: string) => Promise<void> }) {
  const stages = [
    ...
    { id: 'scenarios', label: 'Scenarios', note: 'Compare cap-relief options.' },
...
function BlockedPreflight({ health }: { health: NflDataHealthResponse }) { return <section className="page-shell blocked-page"><span className="blocked-mark">!</span><PageTitle eyebrow="Public data" title="This analysis needs a source refresh" subtitle="The demo will not present a recommendation while a critical source, freshness, or calculation issue remains." /><div className="blocker-list">{health.blockers.map((item) => <p key={item}>{item}</p>)}</div><div className="boundary-note"><strong>What to do next</strong>{health.remediation.map((item) => <p key={item}>{item}</p>)}</div></section>; }
