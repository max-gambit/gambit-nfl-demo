...
test('timeout retains executed narrow receiver query instead of prefetched alternatives',async()=>{
 let round=0;
 const result=await buildNflAiAnswer('Find outside WRs below $6 million current-team cap.',{loadData:loaded,callModel:async()=>{if(round++)throw new Error('Provider unavailable');return response([{name:'search_player_records',input:{team_ids:[],position_groups:['WR'],exclude_nyg:true,numeric_filters:[{field:'cap_2026',operator:'lt',value:6e6}],sort:'cap_asc',limit:10}}]);}});
...
test('executed contract state persists without a model-authored scenario and cannot omit the user budget',async()=>{
 let round=0;
 const result=await buildNflAiAnswer('Convert $6 million for Paulson Adebo. Use a $5 million cap budget and $1 million reserve.',{prefetch:false,loadData:loaded,callModel:async()=>response(round++?[{name:'finish_analysis',input:{answer:'The salary conversion shifts the cap charge into the following season.',answer_statements:['lookup_1:answer'],evidence_id:'lookup_1',continuation_query_id:'lookup_1'}}]:[{name:'calculate_contract_scenario',input:{schema_version:1,season:2026,team_id:'NYG',timing:'post_june_1',moves:[{player_id:'Paulson Adebo',action:'restructure',conversion_amount:6e6}]}}])});
