...
test('new receiver answers lead with analysis and preserve exact facts, methodology and source bindings in detail',async()=>{
 ...
 const result=await buildNflAiAnswer('Compare outside receivers for next-year flexibility.',{prefetch:false,loadData:async()=>({seed,source_mode:'supabase_current_views',fallback_reason:null}),reviewDraft:async()=>[],callModel:async()=>({id:'fixture',type:'message',role:'assistant',model:'fixture',stop_reason:'tool_use',stop_sequence:null,usage:{input_tokens:0,output_tokens:0},content:[{type:'tool_use',id:String(round),name:round++===0?'compare_receivers':'finish_analysis',input:round===1?{priority:'contract_horizon',player_names:['Christian Kirk','Jakobi Meyers','Courtland Sutton','Darnell Mooney','Malik Nabers']}:{answer:conclusion,answer_statements:['lookup_1:answer'],evidence_id:'lookup_1',continuation_query_id:'lookup_1'}}]} as any)});
 ...
 assert.ok(result.body.supporting_details?.some(d=>d.body.includes('Christian Kirk recorded 239 receiving yards')));
