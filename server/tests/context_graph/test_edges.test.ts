...
test('edge extraction creates each relationship edge type from parsed teams', async () => {
  ...
  assert(graph.pickOwnership.some((edge) => edge.owning_team === 'ATL' && edge.owed_team === 'BOS' && edge.year === 2027));
  assert.deepEqual(graph.tradePartners.map((edge) => [edge.team_a, edge.team_b]), [['ATL', 'BOS']]);
  assert.equal(graph.tradePartners[0].trade_count_recent, 2);
  assert(graph.rivalries.some((edge) => edge.team_a === 'ATL' && edge.team_b === 'BOS'));
  assert(graph.personnelConnections.some((edge) => edge.person_name === 'Coach Link' && edge.connected_team === 'BOS'));
  ...
  assert(graph.historicalPursuits.some((edge) => edge.pursuer_team === 'BOS' && edge.target_name === 'Player Alpha'));
});
