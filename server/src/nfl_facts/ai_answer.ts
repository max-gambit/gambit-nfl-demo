...
import { NFL_COMMUNICATION_RULES } from '../nfl_conversation/communication.js';
...
- When the question asks for acquisition costs, funding or a choice between transaction alternatives, name the incoming player's current-year cap charge and cash payment in the opening, with the illustrative basis when applicable. This does not require recapping those costs for a later operational or verification question. Separate costs from additional cap space needed after using the budget and reserve. Say "no salary conversion is needed" when the deal already fits; never lead with "minimum funding is $0" or label a paid acquisition "without funding." A no-acquisition row has zero added obligations because the player is not acquired.
