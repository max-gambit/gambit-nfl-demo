...
export function calculateMinimumFunding(input: NflContractScenarioArgs, available = availableDossiers(), observations=observationsFromScenario(input)): FundingDecision {
  ...
  if(gap===0){decision.kind='no_funding';decision.reason='The acquisition uses '+money(incoming_cap)+' of cap space and '+money(incoming_cash)+' in cash in '+args.season+'. It fits the supplied budget after reserve; no salary conversion is needed.';return decision;}
...
export async function buildCapStrategy(input:NflContractScenarioArgs,observations=observationsFromScenario(input)):Promise<FactualAnswer> {
  ...
  const branches=comp.alternatives.map(a=>({id:a.id,label:a.id==='hold'?'Do not acquire · no added cost':a.id==='requested'?(d.kind==='minimum_conversion'?'Acquire + minimum salary conversion':d.kind==='no_funding'&&input.budget?.type==='cap'?'Acquire · use existing cap room':'Acquire · no salary conversion'):a.id==='acquire_without_funding'?'Acquire · no salary conversion':a.label,result:a.result}));
  const table={title:'Acquisition cost and budget fit',columns:['Alternative','Current cap change vs hold (+ used)','Next-year cap change vs hold (+ used)','Current annual cash change','Next-year annual cash change','After moves and reserve','Funding conversion'],rows:branches.map(a=>{const current=comp.years.find(y=>y.year===args.season)!.alternatives.find(y=>y.alternative_id===a.id)!;const next=comp.years.find(y=>y.year===args.season+1)!.alternatives.find(y=>y.alternative_id===a.id)!;return[a.label,money(current.cap_room_change_vs_hold==null?null:-current.cap_room_change_vs_hold),money(next.cap_room_change_vs_hold==null?null:-next.cap_room_change_vs_hold),money(current.cash_change_vs_hold),money(next.cash_change_vs_hold),money(a.result.budget?.after_reserve??null),a.id==='requested'&&d.selected?d.selected.player_name+': '+money(d.selected.conversion):'None'];}),source_refs:refs};
  const sensitivity={title:'Sensitivity: one input changed at a time',columns:['Illustrative case','Available '+input.budget!.type+' budget','Reserve','New acquisition bonus','Funding gap','Minimum conversion','Next-year cap added by funding','Result'],rows:strategy.sensitivity.map(s=>[s.label,money(s.budget),money(s.reserve),money(s.signing_bonus),money(s.decision.funding_gap),s.decision.selected?s.decision.selected.player_name+': '+money(s.decision.selected.conversion):'None',money(s.decision.selected?.next_year_added_cap??(s.decision.kind==='no_funding'?0:null)),s.decision.kind==='no_funding'?'Fits supplied budget after reserve':s.decision.kind==='minimum_conversion'?'Conditional funding fit':s.decision.reason]),source_refs:refs};
  ...
  answer.body.calculations.unshift({label:input.budget!.type==='cap'?'Additional cap space needed after reserve':'Additional cash budget needed after reserve',formula:'max(0, incoming '+input.budget!.type+' cost + reserve − available budget)',value:money(d.funding_gap),source_refs:refs});
