...
Aggregate market prefetch now requires an aggregate subject and genuine historical or population-comparison wording. Individual contract questions, including those mentioning a position or June 1, remain with the analyst. A failed generation cannot promote unexecuted prefetched material into an answer, tables, sources or saved scenario. Executed evidence remains available when a later model call fails. Regeneration of an incomplete answer rechecks whether its stored historical artifact belongs to the question, preventing the erroneous prefetch from returning. Future provider failures retain their error code and actual generation time; this change does not introduce automatic provider retries or claim to prevent provider outages.

The [original conversation](http://localhost:5175/?conversation=2b2cc837-a4cf-4866-a737-92009d689920) was repaired through the supported regeneration endpoint, preserving brief ID `0d373e2a-c752-4455-91ff-3dacd1db911f`. The corrected answer has 159 words, addresses the requested contract/payroll checks and completed in 43.1 seconds with seven sources. Its scenario state exactly matches the preceding $6m Adebo conversion; the preceding answer's full body is unchanged. A [fresh Chat request](http://localhost:5175/?conversation=f109a9b1-9e10-4334-a8ca-8f31b5343f12), including the same conversion context, completed in 20.2 seconds with 156 words and two sources. Both passed grounding without repair at Astra High/Fast. All displayed references resolve, and neither contains a historical market artifact. Browser checks covered the repaired answer, its source drawer and the fresh answer after reload.

Sixty-five focused tests, server typecheck and application build passed. Regression cases include the exact screenshot wording and contract variants, preservation of genuine historical queries, an injected early provider failure with an unrelated prefetch and a prior contract scenario, and a later failure after an executed conversion. The tests verify saved-state preservation and omission of unrelated tables/sources. Local evidence: `test-results/analyst-restoration/wrong-question-before.json`, `wrong-question-regenerated.json`, `wrong-question-fresh.json`, `wrong-question-verification.json` and test/build/typecheck logs. The original failed record is preserved in those artifacts. The broader comparative acceptance remains unfinished.

## September 9: quieter live analysis

Max's screenshot showed provider-generated task-management narration about tokens, instructions and answer composition. The activity panel now screens complete public summary passages before displaying them. It retains substantive football/contract passages verbatim, removes standalone generated headings, omits process narration and duplicate review steps, and labels the panel simply “Live analysis” / “Analysis.” Real lookup and calculation activity still arrives immediately, including failed operations. The same display projection applies to older saved activity without modifying the stored answer, terms or original summaries.

Summary passages wait for their provider completion event, so later text cannot introduce meta commentary after the opening has already flashed on screen. Explicitly unfinished summaries stay hidden after reload. This is a conservative display filter, not a rewritten model thought or a change to the analyst/reviewer prompt. It can omit a useful passage when that passage also contains task-management language; it does not guarantee every phrasing is detected. It adds no model calls. The final answer retains its existing grounding checks.

The exact screenshot brief `390d1e16-eecf-4596-8dfc-97483856791b` now displays its two executed operations and omits all four process-heavy summaries. Browser inspection passed, and database readback confirmed its full original body is unchanged. A [fresh receiver comparison](http://localhost:5175/?conversation=a1d31cf3-843d-4203-88e4-532ad3374a5e) visibly streamed tool activity before the answer, completed in 51.2 seconds with Astra High/Fast and passed grounding without repair. Its single provider summary was process narration and was omitted; ten tool entries, the answer and chart survived reload. All displayed references resolve among 127 sources. This live run validates tool streaming and screening; substantive summary retention is covered by focused tests, not established by this run.

Sixteen focused presentation/transport/provider tests, server typecheck and application build passed. Cases cover split streaming content without flashes, completion marking, legacy saved activity, whole-passage omission, preserving contract conditions and attributed scouting models, and visible tool failures. Evidence: `test-results/analyst-restoration/stream-prose-verification.json`, `stream-prose-fresh.json`, `stream-prose-before.json`, `stream-prose-tests.log` and build/typecheck logs. The broader comparative acceptance remains unfinished.

## September 9: direct prose across question types

Max requested a durable reduction in meta commentary. A shared communication contract now guides the main writer and semantic reviewer across question types: state the football fact or concrete action, retain a material condition once, and avoid explanations about the calculator, model or what source records cannot certify. Operational follow-ups no longer inherit the requirement to recap acquisition costs. The renderer no longer appends a duplicate generic caveat to a complete, grounding-checked answer; all source caveats remain in Sources, and incomplete answers retain their visible warning.

The exact answer “What must we verify before executing Adebo’s conversion?” was regenerated through the existing endpoint in the [same conversation](http://localhost:5175/?conversation=1503e1c4-8a78-4e58-9fdb-2711752655d4), keeping brief ID `9038379c-def5-44b7-a00a-ccc6ed4c5e58`. Its prose fell from 287 to 159 words. The saved scenario state is exactly unchanged, all twelve source references remain accessible, and browser inspection/reload passed. Its 36.2-second generation passed grounding review without repair. The original answer is preserved in the local verification artifacts.

Two separate live regression cases passed: saved-contract funding (265 words, 38.1 seconds) and a Slayton/Mooney scouting comparison (193 words, 25.7 seconds). Both passed grounded completion, communication checks and source-reference validation. Twenty-four focused unit/regression tests, server and QA typechecks, and the application build passed. The communication detector is a QA signal and a future comparative-acceptance gate, not a production text-deletion filter or a new reason to suppress a useful answer. It exempts explicit methodology questions and preserves attributed models and material financial conditions. These bounded tests do not guarantee every future answer's style.

Reusable live check: `npm --prefix qa-harness run quality:communication -- --live --session-id <existing-Giants-conversation-id>`. `--case verification,funding,scouting` selects cases. It reads existing source data and writes QA artifacts only. The first funding check lacked a conversation scope; the runner now requires that scope before generation. That failed setup run is retained separately, and the scoped rerun passed. Evidence: `test-results/analyst-restoration/communication-verification.json`, `communication-regenerated.json`, `communication-regression/`, and `communication-tests.log`. The broader comparative acceptance below remains unfinished.

## September 9: live reasoning summaries and tool activity

Max asked to connect the previously discussed stream to the UI. New Chat requests now use a streamed POST and request OpenAI's public reasoning summaries. A compact panel opens during generation, shows actual tool activity and summary text as received, and collapses when the checked answer arrives. The bounded summary/activity record is saved with the answer and can be reopened after reload. The numeric and semantic checks still gate the final answer. Raw reasoning text, encrypted state and unfinished answer/tool arguments are never sent to this panel. Existing callers can retain the ordinary JSON endpoint.

The [receiver browser test](http://localhost:5175/?conversation=fad28c8e-1363-45c8-a171-2b126a757478) showed a reasoning summary before a final answer existed, then additional summaries during the same investigation. It completed in 91.7 seconds, with seven saved summaries, nine tool entries, a chart and 108 sources; all displayed source references resolve. Reload retained all seven summaries and the chart. The [Meyers browser test](http://localhost:5175/?conversation=a21ad033-1719-4db5-82ff-b149ff9ad90c) completed in 30.3 seconds with five tool entries and twelve sources; that run returned no summary text. Summary timing and presence depend on the provider, so the UI also reports real executed tool activity. Both answers passed existing grounding checks with Astra High/Fast. These are individual runs, not a latency benchmark.

Transport tests cover fragmented UTF-8 and SSE frames, early summary delivery, excluding raw reasoning/output deltas, incomplete/failed streams without automatic retries, bounded persistence, and the streaming/ordinary HTTP error contracts. All 43 focused transport, saved-contract, analyst and semantic-grounding checks, server typecheck and build passed. A disconnected viewer does not cancel server-side persistence, automatically retry generation or delete the new conversation; an early stream end directs the viewer to reload before retrying. Provider interruption is tested synthetically; a mid-generation browser network outage has not been rehearsed. A follow-up narrowing the receiver question to internal options also opened the panel automatically, completed and collapsed it while retaining the first answer. Local evidence: `test-results/analyst-restoration/stream-readback.json`, `stream-tests-final.log`, `stream-build.log` and `stream-provider-probe.json`. The broader comparative acceptance below remains unfinished; the earlier advisory review helper produced no usable structured verdict and was not rerun.

The implementation follows the [official reasoning summary guide](https://developers.openai.com/api/docs/guides/reasoning#reasoning-summaries) and [streaming event schema](https://developers.openai.com/api/reference/resources/responses/streaming-events#response.reasoning_summary_text.delta).
