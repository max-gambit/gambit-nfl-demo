...
if(phase==='freeze'){
 ...
 await save('manifest.json',{created_at:new Date().toISOString(),saved_contract_fixture_hash:hash(savedRows),candidate_revision:execFileSync('git',['rev-parse','HEAD'],{cwd:root,encoding:'utf8'}).trim(),baseline_revision:'e16ec81',historical_revisions:['32722f3','7c740fc'],model:client.BRIEF_MODEL,effort:'medium',pipeline_effort:{legacy:'low',candidate:'medium'},concurrency:2,deadline_ms:180000,latency_policy:'Quality is the release priority; elapsed time is reported as a tradeoff per user direction.',runtimeHashes,files,question_bank:NFL_QUALITY_CASES,sequences:NFL_QUALITY_SEQUENCES});
...
if(phase==='writing'){
 ...
 await save('writer-context.json',{runtimeHashes,model:client.BRIEF_MODEL,effort:'medium',max_tokens:4500});
