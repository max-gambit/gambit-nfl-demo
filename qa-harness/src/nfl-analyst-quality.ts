...
dotenv.config({path:path.join(root,'server/.env.local')});
...
const analystClient=await moduleAt('server/src/nfl_conversation/model.ts');
...
async function runTurn(id:string,question:string,pipeline:string,history:unknown[]=[]){
 ...
 const callModel=async(params:any,options:any)=>{events.push({request:params});try{const result=await (pipeline==='candidate'?analystClient.createAnalystMessage:client.createClaudeMessage)(params,options);events.push({response:result,model_config:analystClient.analystModelMetadata(result)});return result;}catch(error){events.push({provider_error:String(error)});throw error;}};
 try{const result=await api.buildNflAiAnswer(question,{pipeline,deadlineMs:300000,history,callModel,sessionId:'00000000-0000-4000-8000-000000000003',readSavedContract:async(request:any,context:any)=>savedContracts.selectSavedNflContract(savedRows,request,context),loadTradeSnapshot:async()=>(await trades.loadReviewedNflTransactionSnapshot()).snapshot,onEvidence:(e:unknown[])=>{evidence=e;},onTrace:(e:unknown)=>events.push(e)});const record={id,question,pipeline,history,runtime_hash:hash(runtimeHashes),result,evidence,events,elapsed_ms:Date.now()-started,evidence_hash:hash(evidence)};await save(id+'.json',record);console.log(id,result.body.ai_analysis?.outcome,record.elapsed_ms);return record;}
...
if(phase==='freeze'){
 ...
 await save('manifest.json',{created_at:new Date().toISOString(),saved_contract_fixture_hash:hash(savedRows),candidate_revision:execFileSync('git',['rev-parse','HEAD'],{cwd:root,encoding:'utf8'}).trim(),baseline_revision:'e16ec81',historical_revisions:['32722f3','7c740fc'],model:analystClient.ANALYST_MODEL,effort:analystClient.ANALYST_EFFORT,service_tier:analystClient.ANALYST_SERVICE_TIER,pipelines:{legacy:{model:client.BRIEF_MODEL,effort:'low'},candidate:{model:analystClient.ANALYST_MODEL,effort:analystClient.ANALYST_EFFORT,service_tier:analystClient.ANALYST_SERVICE_TIER}},concurrency:2,deadline_ms:300000,latency_policy:'Quality is the release priority; elapsed time is reported as a tradeoff per user direction.',runtimeHashes,files,question_bank:NFL_QUALITY_CASES,sequences:NFL_QUALITY_SEQUENCES});
...
if(phase==='writing'){
 ...
 await save('writer-context.json',{runtimeHashes,model:analystClient.ANALYST_MODEL,effort:analystClient.ANALYST_EFFORT,service_tier:analystClient.ANALYST_SERVICE_TIER,max_output_tokens:analystClient.ANALYST_MAX_OUTPUT_TOKENS});
 ...
 try{const response=await analystClient.createAnalystMessage({model:analystClient.ANALYST_MODEL,max_tokens:4500,output_config:{effort:analystClient.ANALYST_EFFORT},system:String(prompt)+'\n\nCONTROLLED WRITING REPLAY: All investigation has already run. Use only the identical supplied evidence and context. Adapt output format only: return the answer as prose with [source-ref] citations, alternatives and follow-ups when your writing contract requests them. No tools are available. Do not describe this evaluation. This is a historical-prompt replay, not the original application.',messages:[{role:'user',content:JSON.stringify(bundle)}]},{timeout:180000,maxRetries:0});await save(id+'.json',{id,variant,trial,question:c.question,evidence_hash:bundle.evidence_hash,answer:response.content.filter((b:any)=>b.type==='text').map((b:any)=>b.text).join('\n'),elapsed_ms:Date.now()-start,model:response.model,model_config:analystClient.analystModelMetadata(response),usage:response.usage,stop_reason:response.stop_reason});console.log(id,'done',Date.now()-start);}
